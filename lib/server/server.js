    ...
    constructor() {
        ...
        this.route("/actions/:actionId/form", this.jsonKeepAlive(async (req, complete) => {
            ...
            if ((0, hub_1.isDelegateOauthAction)(action) && request.params.test) {
                form = await action.oauthCheck(request);
            }
        ...
        this.app.get("/actions/:actionId/oauth", async (req, res) => {
            ...
            if ((0, hub_1.isOauthAction)(action) || (0, hub_1.isOauthActionV2)(action)) {
                ...
                const isTargetAction = action.usesCsrfProtection === true;
                ...
                if (isTargetAction) {
                    ...
                    const url = await action.oauthUrl(this.oauthRedirectUrl(action), combinedState);
                    res.redirect(url);
                }
                else {
        ...
        this.app.get("/actions/:actionId/oauth_check", async (req, res) => {
            ...
            if ((0, hub_1.isOauthAction)(action) || (0, hub_1.isOauthActionV2)(action)) {
                const check = await action.oauthCheck(request);
                res.json(check);
            }
            else {
                res.statusCode = 404;
        ...
        this.app.get("/actions/:actionId/oauth_redirect", async (req, res) => {
            ...
            try {
                const isTargetAction = action.usesCsrfProtection === true;
                let originalState = req.query.state;
                if (isTargetAction) {
                ...
                if ((0, hub_1.isOauthAction)(action)) {
                    await action.oauthFetchInfo(modifiedQuery, this.oauthRedirectUrl(action));
                    res.statusCode = 200;
                    res.send(`<html><script>window.close()</script><body>You may now close this tab.</body></html>`);
                }
                else if ((0, hub_1.isOauthActionV2)(action)) {
                    const redirUrl = await action.oauthHandleRedirect(modifiedQuery, this.oauthRedirectUrl(action));
                    if (redirUrl === "") {
                        res.statusCode = 200;
                        res.send(`<html><script>window.close()</script><body>You may now close this tab.</body></html>`);
                    }

...
class GoogleDriveAction extends Hub.OAuthActionV2 {
    ...
    async oauthExtractTokensFromStateJson(stateJson, requestWebhookId) {
        ...
        try {
            ...
            winston.error(`Failed to parse state_json`, { webhookId: requestWebhookId });
            return null;
        }
        ...
        if (state.cid && state.payload) {
            ...
            try {
                ...
                winston.error(`Failed to decrypt or parse encrypted payload: ${e.message}`, { webhookId: requestWebhookId });
